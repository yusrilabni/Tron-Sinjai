...
const openAction = (item: any) => {
  ...
  if (item.tanggal_mulai) {
    try {
      ...
      if (!isNaN(d.getTime())) {
        ...
        startDateStr = String(item.tanggal_mulai).split('T')[0]
      }
    } catch(e) {
      startDateStr = String(item.tanggal_mulai).split('T')[0]
    }
  ...
  updateForm.durasi = parseInt(item.durasi) || 1
  updateForm.satuan = item.satuan || 'HARI'
...
const handleUpdate = async () => {
  ...
  if (updateCaraDurasi.value === 'TANGGAL_AKHIR') {
    if (!updateTanggalAkhir.value) return showToast('Pilih tanggal akhir tayang.', 'error')
    if (updateForm.durasi <= 0) return showToast('Tanggal akhir tayang harus setelah tanggal mulai tayang.', 'error')
  }
  updating.value = true
  try {
    ...
    if (res.success) { 
      showToast('Berhasil memperbarui data tayang & status!', 'success')
      activeSub.value = null
      fetchGallery() 
    } else {
      showToast('Gagal: ' + (res.message || 'Gagal memperbarui'), 'error')
    }
  } catch (e: any) {
